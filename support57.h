 ...
 * - IDLE: the display is static and the calculator is waiting for user input.
 *   The client can safely stop running the emulator until the next key press or
 *   key release.
 * - SLOW: the calculator is not IDLE and is showing some information on the
 *   display: pausing, tracing, or the display blinking. The client should run
 *   the emulator at a speed close to the original one.
 * - FAST: the calculator is fully busy and computing as fast as possible. The
 *   client may run the emulator faster than the actual calculator without the
 *   user losing any meaningful information.
 */
...
/** Returns the current speed of the calculator. */
