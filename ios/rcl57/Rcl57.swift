...
/**
 * The class used to interface with the emulator.
 *
 * This includes accessing the display, the registers, and the program steps. From this class, one
 * can interact with the calculator, notably by pressing and releasing keyboard keys.
 */
class Rcl57 {
    ...
    /// Returns `true` if a given emulator option flag is set. See `rcl57.h`.
    func getOptionFlag(option: Int32) -> Bool {
        return rcl57.options & option != 0
    }

    /// Sets or clears a given emulator option flag. See `rcl57.h`.
    ...
    private func rawData(atURL url: URL) -> UnsafePointer<CChar>? {
        ...
        do {
            try fileRawData = Data(contentsOf: url)
        } catch {
            return nil
        }
    ...
    private init(filename: String) {
        ...
        guard let fileRawBuffer = rawData(atURL: fileURL) else {
            rcl57_init(&rcl57)
            return
        }
    ...
    /// (see `log57.h`) and `index` shoule be in the interval:
